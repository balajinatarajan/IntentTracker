    ...
    content.innerHTML = [...eventLog].reverse().map(e => {
      ...
      switch (e.type) {
        ...
        case 'booking_complete': {
          ...
          detail = `${e.destinationId || ''} <span class="__ik-step-pill">step ${idx}/4 · ${step}</span>`;
          break;
        }
        case 'add_to_cart': {
        ...
        case 'checkout_abandoned': {
          const step = e.funnelStep || '?';
          const idx = typeof e.stepIndex === 'number' ? e.stepIndex + 1 : '?';
          const reason = e.reason ? ` · ${e.reason.replace(/_/g, ' ')}` : '';
          detail = `${e.destinationId || ''} <span class="__ik-step-pill">abandoned step ${idx}/4 · ${step}${reason}</span>`;
          break;
        }
      }
